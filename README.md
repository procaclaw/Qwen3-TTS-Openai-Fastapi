...
- Streaming speed values other than `1.0` are currently coerced to `1.0`
...
- `TTS_COMPILE_MODE` - Unified compile mode for official backend optimization init (shared by streaming and buffered paths): `reduce-overhead` (default) or `max-autotune`
- `TTS_COMPILE_TALKER` - Compile talker forward during optimization init: `true` or `false` (default: `true`)
